...
- `openai` SDK (креды хакатона); NVIDIA — второй провайдер через тот же SDK со сменой `base_url`. Порядок задаёт `LLM_PROVIDERS` (по умолчанию `openai,nvidia`), цепочка видна в `GET /providers`, ответ содержит `llm_provider`/`llm_model`. Любой OpenAI-совместимый эндпоинт (NIM, vLLM, Ollama) добавляется конфигом без кода
- Порт: `8001`
...
- Таймаут 9 с через `asyncio.wait_for` как общий дедлайн цепочки: быстро упавший провайдер (401) отдаёт всё оставшееся время следующему, а не последний провайдер ограничен 75% остатка, чтобы зависший основной не съел время запасного.
