...
import type {Employee,Factor,Gap,History,ImportReport,Overview,Profile,RecResponse,Recommendation} from "./types";
...
export async function recommendations(id:string):Promise<RecResponse>{
  if(useMocks)return mocks.getRecommendations(id);
  const raw=obj(await request(`/api/v1/employees/${encodeURIComponent(id)}/recommendations?lang=ru`));
  if(Array.isArray(raw.recommendations))return raw as unknown as RecResponse;
