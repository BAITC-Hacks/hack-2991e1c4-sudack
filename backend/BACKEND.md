...
Это технический контракт для Go-бэкенда и Python-сервиса рекомендаций. Сейчас в репозитории есть каркас Go, миграция SQLite, именованные SQL-запросы и рабочий GET /healthz. Остальные HTTP-обработчики, импорт, авторизация и вызов Python **ещё не реализованы**. Стартового датасета в репозитории нет: точные названия полей и порядок грейдов нужно сверить с README кита до реализации импортера.
