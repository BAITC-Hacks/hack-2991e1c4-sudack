...
Это технический контракт для Go-бэкенда и Python-сервиса рекомендаций. Сейчас в репозитории есть каркас Go, миграция SQLite и именованные SQL-запросы. HTTP-обработчики, импорт, авторизация и вызов Python **ещё не реализованы**. Стартового датасета в репозитории нет: точные названия полей и порядок грейдов нужно сверить с README кита до реализации импортера.

Демо-сценарий: загрузить кит и дополнительные профили жюри → открыть сотрудника → получить 1–3 объяснимых шага → увидеть прогноз изменения навыков → завершить активность → увидеть новый прогресс и HR-срез. Геймификация в этот путь не входит.

## Стек и границы ответственности

| Компонент | Ответственность |
| --- | --- |
| Go 1.26.1 + Echo v4 | HTTP, права доступа, валидация, импорт, выбор допустимых событий, расчёт прогресса, транзакции |
...
Импорт выполняется одной транзакцией: сначала провалидировать все строки и ссылки, затем записать навыки и грейды, требования, события и эффекты, сотрудников и текущие навыки, историю. Существующий объект с идентичными полями — no-op; объект с тем же ID и другими полями — 409. Это защищает заработанный прогресс от перезаписи. Дополнительная загрузка не удаляет данные. Ошибки должны содержать имя файла и номер строки. Размер должен покрывать минимум 200 сотрудников, 40 событий, 60 навыков и 24 месяца истории плюс профили жюри.

## Общие правила API

Базовый путь /api/v1. Тела запросов и ответов — JSON UTF-8; даты — ISO 8601 UTC. Ошибка:

    {"error":{"code":"invalid_input","message":"...","details":[{"field":"...","reason":"..."}]}}

Коды: 400 — неверный формат; 401 — нет идентификации; 403 — недостаточно прав; 404 — неизвестный ID; 409 — конфликт или повторное завершение; 422 — нарушены связи данных; 503 — сервис рекомендаций недоступен.
...
Публичная проверка процесса и SQLite. Выполнить SELECT 1. Ответ 200: {"status":"ok","database":"ok"}. При недоступной БД — 503. Python не проверять, чтобы healthz оставался быстрым.

### 2. POST /api/v1/imports

**Права:** только HR. multipart/form-data с полями employees, events, skills, history. При первой загрузке нужны все четыре файла; при дополнительной допустимы employees и history, если все ссылки уже есть.

**Логика:** ограничить размер; прочитать файлы; преобразовать поля кита; проверить ID, уровни, gain, аудиторию, порядок грейдов и ссылки; проверить конфликты; открыть транзакцию; вставить новые строки в порядке зависимостей; закрыть транзакцию. При ошибке откатить всё. Изменённый снимок существующего сотрудника вернуть как 409: отдельную операцию административной коррекции можно добавить позже.

**SQL:** поиск существующих записей по первичным ключам; INSERT в skills, grade_levels, grade_requirements, events, event_audience_roles, event_audience_grades, event_skill_effects, employees, employee_skills; для истории INSERT ... ON CONFLICT(source_key) DO NOTHING. Gain из импортированной истории не применять.

Пример ответа:

    {"imported":{"employees":3,"events":0,"skills":0,"history":17},"unchanged":{"employees":0,"history":0}}

### 3. GET /api/v1/employees/:employee_id
...
### 4. GET /api/v1/employees/:employee_id/recommendations
...
### 5. POST /api/v1/employees/:employee_id/events/:event_id/preview
...
### 6. POST /api/v1/employees/:employee_id/events/:event_id/completions
...
Генерация: запустить sqlc generate из internal/repository/sqlite. В текущей среде бинарник sqlc не обнаружен. SQL импорта пока не вынесен в queries.sql: прежде нужно увидеть точную схему кита. Пользовательские значения передавать параметрами, не конкатенацией строк.
...
2. Импорт кита и его повторная загрузка работают без дублей. Дополнительные профили жюри и история загружаются без удаления старых данных.
